...
SSH est sécurisé car il oblige les deux systèmes à s'**authentifier mutuellement** par l'usage de **clés cryptographiques**. Ces clés permettent de **chiffrer** et/ou **signer** les données échangées entre les deux systèmes.

Les clés utilisées pour l'authentification sont dites **asymétriques**, ce qui signifie qu'il y a une clé publique et une clé privée. La clé publique est partagée entre les deux systèmes, tandis que la clé privée est gardée secrète. La **clé publique est utilisée pour vérifier la signature** et **chiffrer les données**, tandis que la **clé privée est utilisée pour signer et déchiffrer les données**. Il est (pour l'instant) impossible de déduire la clé privée à partir de la clé publique. La clé publique est donc partagée sans risque.
...
Une fois que l'authentification est réussie, les systèmes échangent des **clés symétriques** pour chiffrer les données échangées. Ces clés symétriques sont générées aléatoirement pour chaque session. Cela permet d'assurer la **confidentialité** et l'**intégrité** des données échangées, même si un attaquant intercepte les données.
...
sequenceDiagram
    ...
    Client<<-->>Serveur: Établissement de la liaison TCP/IP
    Serveur->>Client: Envoi d'un défi (challenge) chiffré avec la clé publique du client
    Client->>Serveur: Déchiffrement du défi et renvoi du défi déchiffré (avec clé privée)
    Note over Serveur: Vérification du défi déchiffré (avec clé publique) et Authentification
    Client->>Serveur: Établissement de la clé symétrique de session
    Serveur->>Client: Établissement de la clé symétrique de session
    Client<<-->>Serveur: Échange de données chiffrées avec la clé symétrique
</pre>
